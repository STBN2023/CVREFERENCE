    ...
    for (const fileName of files) {
      if (fileName.includes('slide') && fileName.endsWith('.xml')) {
        ...
        if (!file.dir) {
          ...
          if (fileModified) {
            // 2. Nettoyage des placeholders restants pour éviter les labels orphelins
            console.log(`   🧹 [${fileName}] Début du nettoyage des placeholders vides...`);
            placeholderTypes.forEach(placeholder => {
              const escapedPlaceholder = `\\{\\{${placeholder}\\}\\}`;
              // Regex pour trouver un paragraphe <a:p> contenant un placeholder
              const paragraphPattern = new RegExp(`<a:p[^>]*>[\s\S]*?${escapedPlaceholder}[\s\S]*?<\/a:p>`, 'g');
              
              if (paragraphPattern.test(xmlContent)) {
                const initialMatches = (xmlContent.match(paragraphPattern) || []).length;
                xmlContent = xmlContent.replace(paragraphPattern, '');
                console.log(`     → [${placeholder}] ${initialMatches} paragraphes orphelins supprimés.`);
                fileModified = true;
              }
            });
            ...
            replacements++;
            console.log(`   ✅ Fichier ${fileName} modifié et nettoyé`);
          }
        }
      }
    }
